...
AG_simulate_senario_2 = function(seed, inc = c(0.5, 0.75, 1), N, save = TRUE){
  ...
  res = graphical_phyloBB(PrJAP = 0.5, sum_PrMAP = "default", threshold = 0.005, gamma = 0, tree, 
  ...
  if(save == TRUE){
    file = paste("./Result_sim2/sim_", seed, ".RData", sep = "")
    save(res, file = file)
  }else{
    return(res)
  }
...
AG_simulate_senario_2(seed, inc = c(0.5, 1, 1.5), N = 8, save = TRUE)
